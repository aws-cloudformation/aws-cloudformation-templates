...
class PipelineStack(Stack):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        **kwargs) -> None:
        ...
        self.cdk_deploy_project = self.create_cdk_deploy_project(
            project_name=f"{pipeline_config.get_pipeline_name()}-depploy"
        )

        # Create the CodePipeline with source stage
        self.pipeline = self.create_pipeline()

        # Add cdk-nag supressions for expcetions
        self.add_cdk_nag_supressions()
        ...
        return Project(
            ...
            environment=BuildEnvironment(
                ...
                environment_variables={
                    "CDK_DEFAULT_ACCOUNT": BuildEnvironmentVariable(
                        value=self.account
                    ),
                    "CDK_DEFAULT_REGION": BuildEnvironmentVariable(
                        value=self.region
                    ),
                    "NOTIFICATION_EMAIL": BuildEnvironmentVariable(
                        value=self.notification_email
                    ),
                    "DASHBOARD_NAME": BuildEnvironmentVariable(
                        value=self.dashboard_name
                    ),
                    "ALARM_PREFIX": BuildEnvironmentVariable(
                        value=self.alarm_prefix
                    ),
                    "TARGET_ACCOUNTS": BuildEnvironmentVariable(
    ...
    def add_cdk_nag_supressions(self) -> None:
